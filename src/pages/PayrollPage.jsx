...
export default function PayrollPage({ employees, onViewSlip, onAddEmployee, onCreateSlip }) {
  ...
  return (
    <div className="page-shell">
      ...
      <section className="section-block">
        <AppCard title="Employee payroll overview">
          <AppTable
            columns={[
              { label: 'Nama', key: 'name' },
              ...
              { label: 'Action', key: 'action', render: (row) => <AppButton variant="secondary" size="sm" onClick={() => onViewSlip(row)}>Lihat Slip</AppButton> },
            ]}
            data={employees}
      ...
      <AppModal open={createOpen} title="Tambah Slip Baru" onClose={() => setCreateOpen(false)}>
        <div className="form-grid" style={{ gap: '16px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', marginTop: '12px' }}>
            ...
            <AppButton onClick={handleSaveNewSlip}>Buat Slip</AppButton>
          </div>
        </div>
      </AppModal>
    </div>
